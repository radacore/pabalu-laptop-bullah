...
function SearchIndex() {
    const { q, laptops, services, customers, total } = usePage<{
        props: SearchResults;
    }>().props as unknown as SearchResults;
                    ...
                    >
                        {laptops.map((laptop) => (
                            <Link
                                ...
                                className="flex items-center gap-4 border-b border-outline-variant px-5 py-4 transition-colors last:border-b-0 hover:bg-surface-container-low"
                            >
                                <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-primary-container text-on-primary-container">
                                ...
                                <div className="flex flex-shrink-0 items-center gap-2">
                                    {laptop.status?.name && (
                                        <span className="rounded-full bg-surface-container px-2.5 py-1 text-[11px] font-semibold text-on-surface-variant uppercase">
                                            {laptop.status.name}
                                        </span>
                                    )}
                    ...
                    >
                        {services.map((service) => (
                            <Link
                                ...
                                className="flex items-center gap-4 border-b border-outline-variant px-5 py-4 transition-colors last:border-b-0 hover:bg-surface-container-low"
                            >
                                <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-lg bg-secondary-container text-on-secondary-container">
                                    <span className="material-symbols-outlined">
                                        build
                                    </span>
                                </div>
                                <div className="min-w-0 flex-1">
                                    ...
                                    <p className="mt-0.5 truncate text-xs text-on-surface-variant">
                                        ...
                                        {service.complaint &&
                                            ` · ${service.complaint.slice(0, 60)}${service.complaint.length > 60 ? '...' : ''}`}
                                    </p>
                                </div>
                                <div className="flex flex-shrink-0 items-center gap-2">
                                    {service.status?.name && (
                    ...
                    >
                        {customers.map((customer) => (
                            <Link
                                ...
                                className="flex items-center gap-4 border-b border-outline-variant px-5 py-4 transition-colors last:border-b-0 hover:bg-surface-container-low"
                            >
                                <div className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-tertiary-container text-sm font-bold text-on-tertiary-container">
                                ...
                                <span className="material-symbols-outlined flex-shrink-0 text-outline">
                                    chevron_right
                                </span>
                            </Link>
                        ))}
