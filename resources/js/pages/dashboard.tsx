...
function StatusPill({
    ...
    return (
        <span
            className={`inline-flex items-center gap-1.5 rounded-full ${t.bg} ${t.text} px-2.5 py-0.5 text-[11px] font-semibold`}
        >
    ...
    return (
        <div className="group flex flex-col gap-3 rounded-xl border border-slate-200/60 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04)] transition-all hover:border-slate-200 hover:shadow-[0_4px_12px_rgba(15,23,42,0.06)]">
            <div className="flex items-start justify-between">
                ...
                <div
                    className={`flex h-9 w-9 items-center justify-center rounded-lg ${iconBg} ${iconText}`}
                >
                    <Icon className="size-[18px]" />
                </div>
            </div>
            <p className="text-[28px] leading-none font-extrabold tracking-tight text-slate-900">
                {value}
            </p>
    ...
    return (
        <div className="relative overflow-hidden rounded-xl bg-slate-900 p-5 text-white shadow-[0_4px_18px_rgba(15,23,42,0.18)]">
            ...
            <div className="relative mt-3 flex items-center gap-2 text-xs text-white/70">
                <span
                    className={`inline-flex h-1.5 w-1.5 rounded-full ${isPositive ? 'bg-emerald-400' : 'bg-rose-400'} animate-pulse`}
                />
                <span>{hint}</span>
            </div>
        </div>
...
function BarChart({
    ...
    data: { sales: number[]; service: number[]; months?: string[] };
    height?: number;
}) {
    ...
    return (
        ...
        >
            <div className="flex w-12 flex-col justify-between pr-3 pb-7 text-[10px] font-medium text-slate-400">
                {ticks.map((tick, i) => (
            ...
            <div className="relative flex flex-1 flex-col">
                ...
                <div className="relative z-10 flex h-[calc(100%-1.75rem)] items-end justify-between gap-2">
                    {months.map((month, i) => {
                        const sH =
                            Math.max((data.sales[i] ?? 0) / max, 0.01) * 100;
                        const svH =
                            Math.max((data.service[i] ?? 0) / max, 0.01) * 100;
                ...
                <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-sm text-slate-500">
                            Selamat datang kembali. Berikut ringkasan terkini
                            operasional Anda.
                        </p>
                    </div>
...
Dashboard.layout = (page: ReactNode) => (
    <AppLayout breadcrumbs={[{ title: 'Dashboard', href: dashboardRoute() }]}>
        {page}
    </AppLayout>
);
