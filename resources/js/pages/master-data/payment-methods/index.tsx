...
function buildUrl(page: number, search: string | undefined) {
    ...
    return query ? `/master-data/payment-methods?${query}` : '/master-data/payment-methods';
}

const PaymentMethodsIndex = ({ paymentMethods, filters }: PaymentMethodsIndexProps) => {
    const [toDelete, setToDelete] = useState<PaymentMethod | null>(null);
    const [search, setSearch] = useState(filters.search ?? '');

    function applySearch(value: string) {
        setSearch(value);
        router.get('/master-data/payment-methods', { search: value || undefined }, { preserveState: true, replace: true });
    }
                            ...
                            <table className="min-w-full divide-y divide-slate-200">
                                ...
                                <tbody className="divide-y divide-slate-100 bg-white">
                                    {paymentMethods.data.map((method) => (
                                        <tr key={method.id} className="transition-colors hover:bg-slate-50">
                                            <td className="px-6 py-4 text-sm font-medium text-slate-900">
                                                {method.name}
                                            </td>
                                            <td className="px-6 py-4">
                                                <StatusBadge status={method.is_active ? 'tersedia' : 'error'} />
                                            </td>
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex items-center justify-end gap-1">
                                                    <Button asChild variant="success" size="sm">
                                                        <Link href={`/master-data/payment-methods/${method.id}/edit`}>
                                                            <PencilSimple className="mr-1 size-4" weight="bold" />
                                                            Edit
                                                        </Link>
                                                    </Button>
                                                    <Button variant="destructive" size="sm" onClick={() => setToDelete(method)}>
