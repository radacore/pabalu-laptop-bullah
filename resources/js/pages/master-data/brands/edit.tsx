...
interface EditProps {
    brand: { id: number; name: string; is_active: boolean; description?: string | null; };
}

export default function MerekEdit({ brand }: EditProps) {
    const form = useForm({
        name: brand.name || '',
    ...
    function submit(e: { preventDefault: () => void }) {
        e.preventDefault();
        form.put(`/master-data/brands/${brand.id}`);
    }
            ...
            <div className="flex h-full flex-1 flex-col gap-6 overflow-x-auto rounded-xl p-4">
                <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div className="space-y-1">
                        <h1 className="text-2xl font-semibold tracking-tight">Edit Merek</h1>
                        <p className="text-sm text-muted-foreground">Perbarui brand details</p>
                    </div>
                ...
                <form onSubmit={submit} className="space-y-6">
                    <Card>
                        ...
                        <CardContent className="space-y-4">
                            <div className="grid gap-2">
                                ...
                                <Input id="name" value={form.data.name} onChange={(e) => form.setData('name', e.target.value)} placeholder="Enter brand name" />
                    ...
                    <div className="flex justify-end gap-3">
                        ...
                        <Button type="submit" disabled={form.processing}>
                            <Save className="mr-2 size-4" />Perbarui
                        </Button>
                    </div>
                </form>
            </div>
