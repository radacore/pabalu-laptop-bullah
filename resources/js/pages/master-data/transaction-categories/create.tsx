...
export default function TransactionKategoriBuat() {
    const form = useForm({ name: '', type: 'income', is_active: true, description: '' });

    function submit(e: { preventDefault: () => void }) {
        e.preventDefault();
        form.post('/master-data/transaction-categories');
    }

    return (
        <>
            <Head title="Buat Transaksi Kategori" />
            <div className="flex h-full flex-1 flex-col gap-6 overflow-x-auto rounded-xl p-4">
                <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    <div className="space-y-1">
                        <h1 className="text-2xl font-semibold tracking-tight">Buat Transaksi Kategori</h1>
                        <p className="text-sm text-muted-foreground">Add a new income/expense category</p>
                    </div>
                ...
                <form onSubmit={submit} className="space-y-6">
                    ...
                    <div className="flex justify-end gap-3">
                        ...
                        <Button type="submit" disabled={form.processing}><Save className="mr-2 size-4" />Save</Button>
                    </div>
                </form>
            </div>
        </>
