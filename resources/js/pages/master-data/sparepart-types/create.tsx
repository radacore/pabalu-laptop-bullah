...
const SparepartTypesCreate = () => {
    ...
    function submit(event: React.FormEvent) {
        ...
        form.post('/master-data/sparepart-types');
    }

    return (
        <>
            <Head title="Tambah Tipe Sparepart" />
            <div className="mx-auto flex max-w-3xl flex-col gap-6 p-6">
                <header className="flex items-end justify-between">
                    <div>
                        <h2 className="text-3xl font-extrabold tracking-tight text-slate-900">
                            Tambah Tipe Sparepart
                        </h2>
                        <p className="mt-1.5 text-sm text-slate-500">
                            Tambahkan kategori komponen baru
                        </p>
                    </div>
...
SparepartTypesCreate.layout = (page: ReactNode) => (
    <AppLayout
        breadcrumbs={[
            { title: 'Dashboard', href: dashboard() },
            { title: 'Data Master', href: '/master-data' },
            { title: 'Tipe Sparepart', href: '/master-data/sparepart-types' },
            { title: 'Tambah', href: '/master-data/sparepart-types/create' },
        ]}
    >
        {page}
    </AppLayout>
...
export default SparepartTypesCreate;
