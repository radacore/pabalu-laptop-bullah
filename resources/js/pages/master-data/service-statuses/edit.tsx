...
interface ServiceStatusesEditProps {
...
const ServiceStatusesEdit = ({ serviceStatus }: ServiceStatusesEditProps) => {
    const form = useForm({
        name: serviceStatus.name || '',
        color: serviceStatus.color ?? '',
        is_active: serviceStatus.is_active ?? true,
        description: serviceStatus.description ?? '',
    });

    function submit(event: React.FormEvent) {
        event.preventDefault();
        form.put(`/master-data/service-statuses/${serviceStatus.id}`);
    }

    return (
        <>
            <Head title="Edit Status Servis" />
                ...
                <header className="flex items-end justify-between">
                    <div>
                        ...
                        <p className="mt-1.5 text-sm text-slate-500">
                            Perbarui detail tahapan alur servis
                        </p>
                    </div>
...
ServiceStatusesEdit.layout = (page: ReactNode) => (
    <AppLayout
        breadcrumbs={[
...
export default ServiceStatusesEdit;
