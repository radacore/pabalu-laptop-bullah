...
export default function ServiceStatusBuat() {
    const f = useForm({ name: "", is_active: true, description: "" });
    function submit(e: React.FormEvent) { e.preventDefault(); f.post("/master-data/service-statuses"); }
    return (<><Head title="Buat" /><div className="flex h-full flex-1 flex-col gap-6 overflow-x-auto rounded-xl p-4">
        <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div><h1 className="text-2xl font-semibold tracking-tight">Buat Status Servis</h1><p className="text-sm text-muted-foreground">Tambah status servis baru</p></div>
        ...
        <form onSubmit={submit} className="space-y-6">
            ...
            <div className="flex justify-end gap-3"><Button type="button" variant="outline" asChild><Link href="/master-data/service-statuses">Batal</Link></Button><Button type="submit" disabled={f.processing}><Save className="mr-2 size-4" />Save</Button></div>
        </form>
    </div></>);
}