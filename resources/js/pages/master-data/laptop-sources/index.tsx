...
interface IndexProps { laptopSources: IP; filters: { search?: string }; }

export default function LaptopSourceIndex({ laptopSources: data, filters }: IndexProps) {
    const [did, setDid] = useState<number | null>(null);
    const [q, setQ] = useState(filters.search ?? '');
    function hd() { if (!did) return; router.delete(`/master-data/laptop-sources/${did}`, { preserveState: true, replace: true, onSuccess: () => setDid(null) }); }
    function hs(v: string) { setQ(v); router.get('/master-data/laptop-sources', { search: v || undefined }, { preserveState: true, replace: true }); }
    function gp(p: number) { router.get('/master-data/laptop-sources?page='+p, {}, { preserveState: true, replace: true }); }
    return (<><Head title="Sumber Laptop" /><div className="flex h-full flex-1 flex-col gap-6 overflow-x-auto rounded-xl p-4">
        <HalamanHeader title="Sumber Laptop" description="Manage laptop sources" actions={[<Button key="c" asChild><Link href="/master-data/laptop-sources/create"><Plus className="mr-2 size-4" />Add</Link></Button>]} />
        <Card className="overflow-hidden py-0">
            <div className="border-b px-4 py-4 sm:px-6"><div className="relative w-full sm:max-w-sm">
                <input type="search" placeholder="Cari sumber laptop..." className="border-input placeholder:text-muted-foreground focus-visible:border-ring focus-visible:ring-ring/50 w-full rounded-md border bg-transparent px-3 py-2 pl-9 text-sm shadow-xs outline-none focus-visible:ring-[3px]" value={q} onChange={(e) => hs(e.target.value)} />
                    ...
                    <tbody>{data.data.map((it: ItemData) => (<tr key={it.id}>
                        ...
                        <td className="px-4 py-3 text-right sm:px-6">
                            <div className="flex items-center justify-end gap-1">
                                <Button variant="success" size="sm" asChild><Link href={"/master-data/laptop-sources/"+it.id+"/edit"}><Edit className="mr-1 size-4" />Edit</Link></Button>
