...
interface IndexProps { laptopStatuses: IP; filters: { search?: string }; }

export default function LaptopStatusIndex({ laptopStatuses: data, filters }: IndexProps) {
    const [did, setDid] = useState<number | null>(null);
    const [q, setQ] = useState(filters.search ?? '');
    function hd() { if (!did) return; router.delete(`/master-data/laptop-statuses/${did}`, { preserveState: true, replace: true, onSuccess: () => setDid(null) }); }
    function hs(v: string) { setQ(v); router.get('/master-data/laptop-statuses', { search: v || undefined }, { preserveState: true, replace: true }); }
    function gp(p: number) { router.get('/master-data/laptop-statuses?page='+p, {}, { preserveState: true, replace: true }); }
    return (<><Head title="Status Laptop" /><div className="flex h-full flex-1 flex-col gap-6 overflow-x-auto rounded-xl p-4">
        <HalamanHeader title="Status Laptop" description="Manage laptop statuses" actions={[<Button key="c" asChild><Link href="/master-data/laptop-statuses/create"><Plus className="mr-2 size-4" />Add</Link></Button>]} />
                    ...
                    <tbody>{data.data.map((it: ItemData) => (<tr key={it.id}>
                        ...
                        <td className="px-4 py-3 text-right sm:px-6">
                            <div className="flex items-center justify-end gap-1">
                                <Button variant="success" size="sm" asChild><Link href={"/master-data/laptop-statuses/"+it.id+"/edit"}><Edit className="mr-1 size-4" />Edit</Link></Button>
