...
function buildUrl(page: number, search: string | undefined) {
    ...
    return query ? `/master-data/laptop-statuses?${query}` : '/master-data/laptop-statuses';
}

const LaptopStatusesIndex = ({ laptopStatuses, filters }: LaptopStatusesIndexProps) => {
    const [toDelete, setToDelete] = useState<LaptopStatus | null>(null);
    const [search, setSearch] = useState(filters.search ?? '');

    function applySearch(value: string) {
        setSearch(value);
        router.get('/master-data/laptop-statuses', { search: value || undefined }, { preserveState: true, replace: true });
    }
                                            ...
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex items-center justify-end gap-1">
                                                    <Button asChild variant="success" size="sm">
                                                        <Link href={`/master-data/laptop-statuses/${status.id}/edit`}>
