...
interface EP { laptopStatus: MasterData; }
export default function LaptopStatusEdit({ laptopStatus: item }: EP) {
    const f = useForm({ name: item.name||"", is_active: item.is_active??true, description: item.description||"" });
    function submit(e: React.FormEvent) { e.preventDefault(); f.put("/master-data/laptop-statuses/"+item.id); }
    return (<><Head title="Edit Status Laptop" /><div className="flex h-full flex-1 flex-col gap-6 overflow-x-auto rounded-xl p-4">
        <header className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div><h1 className="text-2xl font-semibold tracking-tight">Edit Status Laptop</h1><p className="text-sm text-muted-foreground">Perbarui</p></div>
        ...
        <form onSubmit={submit} className="space-y-6">
            ...
            <div className="flex justify-end gap-3">
                <Button type="button" variant="outline" asChild><Link href="/master-data/laptop-statuses">Batal</Link></Button>
