...
interface LaptopStatusesEditProps {
...
const LaptopStatusesEdit = ({ laptopStatus }: LaptopStatusesEditProps) => {
    const form = useForm({
        name: laptopStatus.name || '',
        color: laptopStatus.color ?? '',
        is_active: laptopStatus.is_active ?? true,
        description: laptopStatus.description ?? '',
    });

    function submit(event: React.FormEvent) {
        event.preventDefault();
        form.put(`/master-data/laptop-statuses/${laptopStatus.id}`);
    }

    return (
        <>
            <Head title="Edit Status Laptop" />
                ...
                <header className="flex items-end justify-between">
                    <div>
                        ...
                        <p className="mt-1.5 text-sm text-slate-500">
                            Perbarui detail label status inventaris
                        </p>
                    </div>
...
LaptopStatusesEdit.layout = (page: ReactNode) => (
    <AppLayout
        breadcrumbs={[
...
export default LaptopStatusesEdit;
