...
function buildTransactionsHref(page: number, filters: Props['filters']) {
    ...
    return query ? `/financial-transactions?${query}` : '/financial-transactions';
}
...
function buildPeriodLabel(filters: Props['filters']) {
    ...
    if (filters.from_date) return `Mulai ${formatTanggal(filters.from_date)}`;
    if (filters.to_date) return `Sampai ${formatTanggal(filters.to_date)}`;
    return 'Semua Periode';
}

function exportVisibleTransactions(transactions: FinancialTransaction[]) {
    const headers = ['Tanggal', 'Kode', 'Deskripsi', 'Kategori', 'Metode Pembayaran', 'Tipe', 'Jumlah'];
    ...
    if (tone === 'balance') {
        ...
        return (
            <div className="relative overflow-hidden rounded-xl bg-slate-900 p-5 text-white shadow-[0_4px_18px_rgba(15,23,42,0.18)]">
                ...
                <p className="relative mt-3 text-[28px] font-extrabold leading-none tracking-tight">
                ...
                <div className="relative mt-3 flex items-center gap-2 text-xs text-white/70">
                    ...
                    />
                    <span>{positive ? 'Cashflow positif' : 'Cashflow negatif'}</span>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="flex flex-col gap-3 rounded-xl border border-slate-200/60 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04)] transition-all hover:border-slate-200 hover:shadow-[0_4px_12px_rgba(15,23,42,0.06)]">
            <div className="flex items-start justify-between">
                ...
                >
                    {isIncome ? <ArrowUpRight className="size-[18px]" /> : <ArrowDownRight className="size-[18px]" />}
                </div>
            </div>
            <p className={`text-[28px] font-extrabold leading-none tracking-tight ${isIncome ? 'text-emerald-700' : 'text-rose-700'}`}>
                {formatCurrency(value)}
            </p>
...
function CashflowChart({ transactions }: { transactions: CashflowTransaction[] }) {
    ...
    transactions.forEach((t) => {
        ...
        else existing.expense += Number(t.amount ?? 0);
    ...
    const months = Array.from(buckets.entries())
        ...
        .map(([key, value]) => {
            const [, m] = key.split('-');
            const label = new Intl.DateTimeFormat('id-ID', { month: 'short' }).format(new Date(2026, Number(m) - 1, 1));
            return { label, ...value };
        });
    ...
    if (months.length === 0) {
        return (
            <div className="flex h-64 flex-col items-center justify-center rounded-lg border border-dashed border-slate-200 text-center">
                <p className="text-sm font-medium text-slate-900">Belum ada data</p>
                <p className="text-xs text-slate-500">Tambahkan transaksi untuk melihat tren.</p>
            </div>
        );
    }
    ...
    return (
        <div className="relative flex h-56 w-full sm:h-64">
            <div className="flex w-14 flex-col justify-between pb-7 pr-3 text-[10px] font-medium text-slate-400">
                {ticks.map((tick, i) => (
...
function CategoryBreakdownCard({ transactions }: { transactions: FinancialTransaction[] }) {
    ...
    const items = Array.from(grouped.entries())
        .map(([name, v]) => ({ name, total: v.income + v.expense, income: v.income, expense: v.expense }))
    ...
    if (items.length === 0 || totalAmount === 0) {
        return (
            <div className="flex h-64 flex-col items-center justify-center rounded-lg border border-dashed border-slate-200 text-center">
                <p className="text-sm font-medium text-slate-900">Belum ada data</p>
                <p className="text-xs text-slate-500">Kategori akan muncul setelah ada transaksi.</p>
            </div>
        );
    }
            ...
            {items.map((item) => {
                ...
                return (
                    <div key={item.name}>
                        ...
                        <p className="mt-1 text-xs font-medium text-slate-500">{formatCurrency(item.total)}</p>
                    </div>
                );
            })}
        </div>
...
const FinancialTransactionsIndex: HalamanComponent = ({ transactions, chart_transactions, filters, summary, categories }) => {
    ...
    function submitFilters(event: { preventDefault: () => void }) {
        ...
        filterForm.get('/financial-transactions', { preserveState: true, replace: true });
    }
        ...
        <>
            ...
            <div className="flex flex-col gap-6 p-8">
                <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        <h2 className="text-3xl font-extrabold tracking-tight text-slate-900">Keuangan</h2>
                    ...
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center">
                        <button
                            type="button"
                            onClick={() => exportVisibleTransactions(transactions.data)}
                ...
                <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                    ...
                    />
                    <SummaryCard title="Saldo (Net)" value={summary.balance} tone="balance" />
                </div>
                ...
                <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
                    <section className="rounded-xl border border-slate-200/60 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04)] lg:col-span-2">
                        <div className="mb-4 flex items-center justify-between">
                            <div>
                                <h3 className="text-base font-semibold text-slate-900">Tren Arus Kas</h3>
                                <p className="text-xs text-slate-500">6 bulan terakhir</p>
                            </div>
                            <div className="flex items-center gap-4 text-xs">
                                <div className="flex items-center gap-1.5">
                                    <span className="h-2.5 w-2.5 rounded-sm bg-emerald-500" />
                                    <span className="font-medium text-slate-600">Pemasukan</span>
                                </div>
                                <div className="flex items-center gap-1.5">
                                    <span className="h-2.5 w-2.5 rounded-sm bg-rose-500" />
                                    <span className="font-medium text-slate-600">Pengeluaran</span>
                                </div>
                            </div>
                        </div>
                    ...
                    <section className="rounded-xl border border-slate-200/60 bg-white p-5 shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
                        <div className="mb-4 flex items-center justify-between">
                            <h3 className="text-base font-semibold text-slate-900">Kategori</h3>
                            <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[11px] font-bold text-slate-600">
                                Top 5
                            </span>
                        </div>
                        <CategoryBreakdownCard transactions={transactions.data} />
                    </section>
                </div>
                ...
                >
                    <div className="grid min-w-44 flex-1 gap-1.5">
                        <label htmlFor="ft-search" className="text-[11px] font-semibold uppercase tracking-wider text-slate-500">
                            Cari
                        </label>
                        <input
                            id="ft-search"
                            ...
                            className="block w-full rounded-lg border border-slate-200 bg-slate-50 px-3.5 py-2 text-sm text-slate-900 placeholder-slate-400 transition-colors focus:border-blue-500 focus:bg-white focus:outline-none focus:ring-1 focus:ring-blue-500"
                        />
                    </div>

                    <div className="grid min-w-40 gap-1.5">
                        <label htmlFor="ft-type" className="text-[11px] font-semibold uppercase tracking-wider text-slate-500">
                            Tipe
                        </label>
                        <div className="relative">
                            <select
                                ...
                                className="block w-full appearance-none rounded-lg border border-slate-200 bg-white px-3.5 py-2 pr-9 text-sm text-slate-900 transition-colors focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                            >
                    ...
                    <div className="grid min-w-48 gap-1.5">
                        <label htmlFor="ft-category" className="text-[11px] font-semibold uppercase tracking-wider text-slate-500">
                            Kategori
                        </label>
                        <div className="relative">
                            <select
                                ...
                                onChange={(e) => filterForm.setData('transaction_category_id', e.target.value)}
                                className="block w-full appearance-none rounded-lg border border-slate-200 bg-white px-3.5 py-2 pr-9 text-sm text-slate-900 transition-colors focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                            >
                    ...
                    <div className="grid w-full gap-1.5 sm:min-w-56">
                        ...
                        <div className="flex flex-col gap-1.5 sm:flex-row sm:items-center">
                            <input
                                ...
                                className="block w-full min-w-0 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 transition-colors focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
                ...
                <section className="overflow-hidden rounded-xl border border-slate-200/60 bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)]">
                    <div className="flex flex-col gap-1 border-b border-slate-100 px-5 py-3.5 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            <h3 className="text-sm font-semibold text-slate-900">Daftar Transaksi</h3>
                            <p className="text-xs text-slate-500">
                                {buildPeriodLabel(filters)} · {transactions.total} catatan
                            </p>
                        </div>
                    </div>
                    ...
                    {transactions.data.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-16 text-center">
                            ...
                            <p className="mt-1 text-sm text-slate-500">Sesuaikan filter atau buat transaksi baru.</p>
                        </div>
                    ) : (
                        <div className="overflow-x-auto">
                            <table className="min-w-full divide-y divide-slate-200">
                                <thead className="bg-slate-50">
                                    <tr>
                                        <th className="px-5 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-slate-500 sm:px-6">
                                            Tanggal
                                        </th>
                                        <th className="px-5 py-3 text-left text-[11px] font-semibold uppercase tracking-wider text-slate-500 sm:px-6">
                                            Kode
                                        ...
                                        <th className="px-5 py-3 text-right text-[11px] font-semibold uppercase tracking-wider text-slate-500 sm:px-6">
                                            Jumlah
                                        </th>
                                        <th className="px-5 py-3 text-right text-[11px] font-semibold uppercase tracking-wider text-slate-500 sm:px-6">
                                            Aksi
                                        ...
                                        return (
                                            ...
                                            >
                                                <td className="whitespace-nowrap px-5 py-3.5 text-sm text-slate-600 sm:px-6">
                                                    {formatTanggal(transaction.transaction_date)}
                                                </td>
                                                <td className="whitespace-nowrap px-5 py-3.5 text-sm font-mono text-slate-700 sm:px-6">
                                                ...
                                                <td
                                                    className={`whitespace-nowrap px-5 py-3.5 text-right text-sm font-bold tabular-nums sm:px-6 ${
                        ...
                        <div className="flex flex-col gap-3 border-t border-slate-100 px-5 py-3.5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
                            ...
                            <div className="flex flex-wrap items-center gap-1">
                                {transactions.current_page > 1 && (
                                    <Link
                                        href={buildTransactionsHref(transactions.current_page - 1, filters)}
                                ...
                                {Array.from(
                                    new Set(
                                        ...
                                        ].filter((p) => p >= 1 && p <= transactions.last_page),
                                    ),
                                ).map((page) =>
                                    page === transactions.current_page ? (
                                        ...
                                        <Link
                                            key={page}
                                            href={buildTransactionsHref(page, filters)}
                                            className="inline-flex items-center rounded-md border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 transition-colors hover:bg-slate-50"
                                        >
                                            {page}
                                        </Link>
...
FinancialTransactionsIndex.layout = (page: ReactNode) => (
    <AppLayout breadcrumbs={[{ title: 'Dashboard', href: '/' }, { title: 'Keuangan', href: financialTransactionsIndex() }]}>
        {page}
    </AppLayout>
);
